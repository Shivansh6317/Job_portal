...
public class AuthService {
    ...
    public String register(RegisterRequest request) {
        ...
        if (request.getRole() != null && !request.getRole().isBlank()) {
            try {
                ...
                if (requested == Role.JOBGIVER || requested == Role.JOBSEEKER) {
                    role = requested;
                } else {
                    throw new CustomException("Cannot assign role", HttpStatus.FORBIDDEN);
                }
            } catch (IllegalArgumentException ex) {
                throw new CustomException("Invalid role", HttpStatus.BAD_REQUEST);
            }
        }
        User user = User.builder().name(request.getName()).email(request.getEmail()).password(passwordEncoder.encode(request.getPassword())).verified(false).role(role).build();
    ...
    public AuthResponse login(LoginRequest request) {
        User user = userRepository.findByEmail(request.getEmail()).orElseThrow(() -> new CustomException("Invalid email or password", HttpStatus.UNAUTHORIZED));
        if (!user.isVerified()) {
